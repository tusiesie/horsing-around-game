...
class AudioManager:
    ...
    def __init__(self):
        ...
        # current_sound_file = os.path.join(self.audio_file, "voice/testSpeech.mp3")
        ...
        # self.voice_sound = pygame.mixer.Sound(current_sound_file)
        # self.voice_sound.play(-1)
            

    def get_volume(self, category):
        """
        Get the volume level of given sound category,
        else return default .5
        """
        return self.volume_levels.get(category, .5)

    def set_volume(self, category, value):
        """
        Updates the volume for the given sound category to value
        """
        self.volume_levels[category] = max(0, min(1, value))
        self.recalibrate()
    ...
    def set_voice_sound(self, category, new_file):
        """
